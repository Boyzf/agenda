...
var registerCmd = &cobra.Command{
	Use:   "register",
	Short: "Register",
	Long:  `Register users for account`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, value := range user_info{
			if value.Username == username{
				fmt.Println("Failed: The Username is already in use!")
				return
			}
		}
		fileptr, error := os.OpenFile("entity/User.json", os.O_APPEND|os.O_WRONLY|os.O_TRUNC,0644)
		defer fileptr.Close()
		...
		error = encoder.Encode(user_info)
		if error != nil{
			fmt.Println(error)
		}
		fmt.Println("Succeed!")	
